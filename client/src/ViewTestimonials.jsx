...
const ViewTestimonials = () => {
  ...
  useEffect(()=>{
    axios.get('https://ggz.onrender.com/testimonials').then((res,err)=>{
      if(err){
        toast.error(`${err}`);
      }
      else{
    ...
    <Carousel>
              {testimonials && testimonials.map(testimonial=>(
                 <div className='testimonials__container'>
                    <img className='avatar__test' style={{height:'60px',width:'60px',borderRadius:'50%'}} src={`https://ggz.onrender.com/${testimonial.photoUrl}`} />
