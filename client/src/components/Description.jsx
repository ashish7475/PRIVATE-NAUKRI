...
const Description = (props) => {
  ...
  useState(()=>{
       ...
       axios.get('http://localhost:5000/jobdetails',{
        ...
        if(err){
          ...
          setPageData(res.data.description.slice(12,res.data.description.length))
  ...
  return (
    ...
    <div className='description__outer'>
      ...
      {title && <div><h2 className='job__title'>{title}</h2></div>}
      {location && <span className='location__span'>{location} | JOB ID: {jobId}</span>}
      <div className='description__body'>
       {pageData && <p> {pageData} </p>}
      {basicQual && <p>{basicQual}</p>}
      {prefQual && <p>{prefQual}</p>}
      {applyUrl && <a href={`${applyUrl}`} target='_blank'><button className='btn btn-primary apply__btn'>Apply</button></a>}
      </div>
